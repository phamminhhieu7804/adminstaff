...
const filePath = path.resolve(__dirname, 'src/tabs/EmployeesTab.jsx');
...
content = content.replace(
    /export default function EmployeesTab\(\) \{\s+const \{ storeId, storeData \} = useStore\(\);/,
...
const replacements = [
    ...
    ["'Thêm nhân viên thành công!'", "t('add_employee_success')"],
    ["'Cập nhật nhân viên thành công!'", "t('update_employee_success')"],
    ...
    ["'Mức lương mới (VNĐ)'", "t('new_salary_vnd')"],
    ["'Số tiền thưởng (VNĐ)'", "t('bonus_amount_vnd')"],
    ["'Số tiền phạt (VNĐ)'", "t('penalty_amount_vnd')"],
    ["\"VD: 30000\"", "t('placeholder_salary_increase')"],
    ["\"VD: 50000\"", "t('placeholder_salary_bonus')"],
    [">Lý do / Ghi chú<", ">{t('reason_note')}<"],
    ["\"VD: Đi trễ, làm hỏng đồ...\"", "t('placeholder_penalty_reason')"],
    ["\"VD: Làm tốt, đạt doanh thu...\"", "t('placeholder_bonus_reason')"],
    [">Xác nhận<", ">{t('confirm')}<"],
