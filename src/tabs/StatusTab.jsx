...
export default function StatusTab() {
  ...
  useEffect(() => {
    ...
    const unEmployees = onSnapshot(query(collection(db, 'employees'), where('storeId', '==', storeData.id)), (snap) => {
      setEmployees(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    
    const unShifts = onSnapshot(query(collection(db, 'shifts'), where('storeId', '==', storeData.id)), (snap) => {
    ...
    const unAdjustments = onSnapshot(query(
      collection(db, 'salary_adjustments'), 
      where('storeId', '==', storeData.id)
    ), (snap) => {
    ...
    const unLogs = onSnapshot(query(collection(db, 'attendance_logs'), where('storeId', '==', storeData.id)), (snap) => {
  ...
  const employeeStats = employees.map(emp => {
    ...
    if (empTodaySchedules.length > 0) {
       ...
       if (missingShifts > 0) {
          isAbsent = true;
          if (empLeaves.length > 0) absentStatus = 'Nghỉ có phép';
          else absentStatus = 'Vắng không phép';
       }
    }
  ...
  const handleSendWarning = async (e) => {
    ...
    try {
      ...
      showToast('success', 'Đã gửi thông báo cảnh cáo tới nhân viên!');
      setSentWarnings(prev => [...prev, warningModal.log.id]);
      setWarningModal({ show: false, log: null, message: '' });
    } catch (err) {
      showToast('error', 'Lỗi khi gửi thông báo');
    }
  };
  ...
  return (
    <div className="h-full flex flex-col relative">
      ...
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Theo Dõi Trạng Thái</h2>
        <p className="text-gray-500 mt-1">Quản lý trạng thái làm việc và gửi cảnh báo đi trễ cho nhân viên.</p>
      </div>
            ...
            >
              ...
              <div className="mb-4 flex flex-col items-start gap-2">
                {emp.isWorking ? (
                  <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-medium", emp.isCurrentlyLate ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700")}>
                    ...
                    Đang làm ca: {emp.activeShift} {emp.isCurrentlyLate && "(Trễ)"}
                  </div>
                ) : (
                ...
                {emp.isAbsent && (
                  <div className={cn("inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold border", emp.absentStatus === 'Nghỉ có phép' ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-red-50 text-red-700 border-red-200")}>
              ...
              <div className="grid grid-cols-3 gap-2 text-sm pt-3 border-t border-gray-100">
                <div className="bg-emerald-50 rounded-lg p-2 flex flex-col items-center justify-center">
                  <p className="text-emerald-700 font-medium">Đúng giờ</p>
                ...
                <div className="bg-amber-50 rounded-lg p-2 flex flex-col items-center justify-center">
                  <p className="text-amber-700 font-medium">Đi trễ</p>
                ...
                <div className="bg-red-50 rounded-lg p-2 flex flex-col items-center justify-center">
                  <p className="text-red-700 font-medium">Bỏ ca</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-gray-50 p-4 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-500">Tháng này: Đúng giờ ({selectedEmployee.totalOnTime}) - Đi trễ ({selectedEmployee.totalLate})</p>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-0">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Thời gian</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Ca làm</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {selectedEmployee.recentLogs.length === 0 ? (
                     <tr><td colSpan="4" className="px-6 py-8 text-center text-gray-500">Không có dữ liệu hoạt động tháng này</td></tr>
                  ) : (
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-center">
                          {log.isSkipped ? (
                            ...
                            <div className="flex items-center justify-center gap-1.5 text-amber-700 text-sm font-medium">
                              <AlertTriangle className="w-4 h-4" /> Trễ {log.lateMinutes} phút
                            </div>
                          ) : (
                            ...
                            <button 
                              onClick={() => {
                                setWarningModal({
                                  show: true,
                                  log,
                                  message: `Nhắc nhở: Bạn đã check-in trễ ${log.lateMinutes} phút vào ca [${log.shiftName}] lúc ${format(log.dateObj, 'HH:mm dd/MM/yyyy')}. Yêu cầu bạn đi làm đúng giờ.`
                                });
                              }}
                            ...
                            <button 
                              onClick={() => {
                                setWarningModal({
                                  show: true,
                                  log,
                                  message: `Nhắc nhở: Bạn đã không đi làm ca [${log.shiftName}] ngày ${format(log.dateObj, 'dd/MM/yyyy')}. Vui lòng giải trình lý do với Quản lý.`
                                });
                              }}
            ...
            <div className="border-t border-gray-200 bg-gray-50">
              <div className="p-4 border-b border-gray-200 flex items-center gap-2">
                ...
                <h4 className="font-bold text-gray-900">Lịch sử điều chỉnh lương</h4>
              </div>
              <div className="p-4 max-h-48 overflow-y-auto space-y-3">
                {salaryAdjustments.filter(a => a.employeeCode === selectedEmployee.employeeCode).length === 0 ? (
                  <p className="text-gray-500 text-center text-sm">Chưa có dữ liệu biến động lương.</p>
                ) : (
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={cn(
                            ...
                            {adj.type === 'increase' ? 'TĂNG LƯƠNG GỐC' : adj.type === 'bonus' ? 'THƯỞNG' : 'PHẠT'}
                          </span>
                          <span className="text-xs text-gray-500">{format(new Date(adj.createdAt), 'HH:mm - dd/MM/yyyy')}</span>
                        </div>
                        {adj.reason && <p className="text-sm text-gray-600 mt-1 line-clamp-2">Lý do: {adj.reason}</p>}
                      </div>
                      <div className={cn(
                        ...
                        {adj.type === 'increase' ? 'Lên ' : (adj.type === 'bonus' ? '+' : '-')}
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-red-50 text-red-900">
              <h4 className="font-bold flex items-center gap-2"><AlertTriangle className="w-5 h-5" /> Soạn cảnh cáo</h4>
            ...
            <form onSubmit={handleSendWarning} className="p-5 space-y-4">
              <p className="text-sm text-gray-600">
                Thông báo này sẽ được gửi trực tiếp đến nhân viên <b>{selectedEmployee?.fullName}</b>.
              </p>
              <div>
